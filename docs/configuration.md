...
A request that does not match the include patterns is never rejected locally before the backend configuration has been loaded: the include decision is always made against the effective (backend-informed) configuration. Only requests to other origins, requests matching a local `exclude` pattern, and requests to the discovery endpoints (the resolved `jweConfigPath` and `jwksPath`, which are always exempt from JWE protection) skip the configuration load. The client default excludes do not skip it — the backend's published `excludedPaths` replace them, so they are only applied as part of the effective configuration. If the backend configuration cannot be loaded, requests to the backend origin that are not locally excluded fail with `JWE_CONFIG_LOAD_FAILED` instead of being sent unprotected — exclude paths that must work without the backend configuration via `exclude`.
