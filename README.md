...
Without options, the backend origin defaults to the frontend's own origin and the discovery paths
default to the application base path (the Angular base href, which matches the backend's servlet
context path when the frontend is served by its backend) plus the well-known paths. For a base href
of `/myapp/`, the client loads:
...
For a cross-origin backend, configure `origin` (and `jweConfigPath` for a backend under a context
path) explicitly.
