...
describe('JeapJweClientConfigService', () => {
  ...
  it('appends client-owned exclude patterns to the client defaults', async () => {
    configure({
      origin: sameOrigin,
      exclude: ['/local-public/**'],
    });
  ...
  it('uses backend-published includedPaths and excludedPaths as the source of truth', async () => {
    configure({
      origin: sameOrigin,
      exclude: ['/local-extra/**'],
    });
    ...
    expect(config.include).toEqual(['/myapp/*api*/**']);
    // Backend excludes are authoritative; local excludes are appended on top.
    expect(config.exclude).toEqual([
      '/myapp/actuator/**',
      '/myapp/.well-known/jwks.json',
      '/myapp/.well-known/jwe-configuration',
  ...
  it('falls back to the default include pattern when the backend omits includedPaths', async () => {
    configure({ origin: sameOrigin });
